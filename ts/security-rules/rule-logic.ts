...
type SerializationOptions = {
    placeholders: RuleLogicPlaceholders
    stack?: RuleLogicStackFrame[]
}
...
export default function serializeRuleLogic(
    logic: RuleLogic,
    options: SerializationOptions,
): string {
    if (isRuleLogicValue(logic)) {
        return serializeValue(logic, {
            ...options,
            stack: [...(options.stack ?? []), { child: logic }],
        })
    } else if (isBinaryOp(logic)) {
        return serializeBinaryOp(logic, {
            ...options,
            stack: [...(options.stack ?? []), { child: logic }],
        })
    } else if (isExists(logic)) {
        return serializeRuleLogic(logic.exists, {
            ...options,
            stack: [...(options.stack ?? []), { child: logic }],
        })
    } else {
        throw new Error(
            `Detected unknown access rule expression: ${JSON.stringify(logic)}`,
        )
    }
}

function serializeValue(
    value: RuleLogicValue,
    options: SerializationOptions,
): string {
    if (typeof value === 'string' && value.charAt(0) === '$') {
        ...
        for (let i = 0; i < pathComponents.length; ++i) {
            ...
            if (placeholder) {
                const restComponents = pathComponents.slice(
...
function serializeBinaryOp(
    op: RuleLogicBinaryOp,
    options: SerializationOptions,
): string {
    ...
    const isChainable = CHAINABLE_BINARY_OPS.has(
        operatorKey as RuleLogicBinaryOpKey,
    )
    if (operands.length < 2) {
        throw new Error(
            `Detected access rule '${operatorKey}' expression with less than 2 operands`,
        )
    }
    if (!isChainable && operands.length !== 2) {
        throw new Error(
            `Detected access rule '${operatorKey}' expression with invalid number of operands: ${JSON.stringify(operands)}`,
        )
    }

    const operator = BINARY_OPS[operatorKey]
    const expression = operands
        .map((operand) => serializeRuleLogic(operand, options))
        .join(` ${operator} `)
    return `(${expression})`
}
